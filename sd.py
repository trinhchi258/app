...
def user_dashboard():
    ...
    btn_view_personal_info = tk.Button(window, text="Xem thông tin cá nhân", command=view_personal_info)
    btn_view_personal_info.pack(pady=5)
...
def admin_dashboard():
    ...
    btn_logout = tk.Button(window, text="Đăng xuất", command=logout)
    btn_logout.pack(pady=5)
...
def view_schedule():
    try:
        with open('schedule.csv', 'r') as file:
            reader = csv.DictReader(file)
            matches = [f"{row['Date']} - {row['Home Team']} vs {row['Away Team']}" for row in reader]
            match_list = "\n".join(matches)
...
def save_new_match(team1, team2, referee, stadium, date_time):
    if team1 and team2 and referee and stadium and date_time:
        with open('matches.csv', 'a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([team1, team2, referee, stadium, date_time])  # Lưu thông tin trận đấu
        messagebox.showinfo("Thành công", "Thêm trận đấu thành công!")
        manage_schedule()
    else:
        messagebox.showwarning("Lỗi", "Vui lòng nhập đầy đủ thông tin!")
def get_team_names():
